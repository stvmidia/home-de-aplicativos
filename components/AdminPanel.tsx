...
import { Plus, Trash2, Save, X, Image as ImageIcon, Link as LinkIcon, Tag as TagIcon } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ projects, onAddProject, onDeleteProject }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-display font-bold text-white">Gerenciar Aplicativos</h2>
          <p className="text-slate-400 mt-1">Adicione ou remova seus projetos do portfolio.</p>
        </div>
        <button
      ...
      {isAdding && (
        <div className="glass-panel p-6 rounded-2xl animate-slide-up border-l-4 border-accent-500">
          <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            <span className="p-1 bg-accent-500/20 rounded">✨</span> Novo Projeto
          </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent transition-all"
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent transition-all resize-none"
              />
            </div>
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid gap-4">
            {projects.map((project) => (
              <div 
                key={project.id}
                className="flex items-center gap-4 bg-slate-800/40 p-4 rounded-xl border border-slate-700/50 hover:border-slate-600 transition-all group"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-400 truncate">{project.url}</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <a 
                    ...
                    className="p-2 text-slate-400 hover:text-brand-400 hover:bg-brand-400/10 rounded-lg transition-colors"
                    title="Testar Link"
                  >
